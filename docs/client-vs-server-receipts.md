...
Client-observed receipts are useful when you control the client boundary. They prove what the client observed sending and receiving.

Server-attested receipts are stronger for third-party proof because they are emitted closer to the source of truth for tool execution. They prove what the server attests happened, assuming the server key and runtime are trusted.
