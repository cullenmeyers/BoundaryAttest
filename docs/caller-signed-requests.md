...
AgentReceipt currently keeps caller identity simple: the host system authenticates the caller, session, service, or agent, then passes caller metadata into AgentReceipt. For server-side tool execution, the server signs a `server_attested` receipt over the tool/action fields and any supplied caller metadata.

That means AgentReceipt can record caller metadata, but it does not verify caller identity by itself.
...
- The host passes caller metadata into AgentReceipt.
- The server executes the tool.
- The server signs a `server_attested` receipt over the tool/action fields.

This is useful when the host runtime is trusted. The limitation is that AgentReceipt records the caller metadata the host supplied; it does not independently prove that the named caller controlled a caller key or authenticated directly to AgentReceipt.
...
This keeps AgentReceipt in the receipt/provenance layer. It does not turn AgentReceipt into an authentication, authorization, or permission system.

## What this proves

Caller-signed requests could prove that the holder of the caller key signed the request intent. Server-attested receipts could prove that the holder of the server key attested to the execution result.
...
Before adding caller-signed requests, AgentReceipt would need explicit answers to several trust and custody questions:
...
These decisions belong to the host deployment and surrounding security model. AgentReceipt can preserve and sign evidence, but it should not imply that evidence is automatically sufficient for authentication or authorization.
...
Treat caller-signed requests as a future optional feature. Keep the MVP host-supplied caller metadata simple for now, and continue to state clearly that AgentReceipt does not solve authentication.
